<template>
  <div class="cart">
    ...
    <div class="cart-box" v-if="cartList.length > 0 && login">
      <!-- 购物车开头 -->
      <div class="cart-title"  @click="isEdit = true" >
      ...
      <div class="footer-fixed">
        <div  class="all-check">
          <van-checkbox :value="isCheckedAll" @click="toggleAllCheck" icon-size="18"></van-checkbox>
          全选
        </div>
        ...
        <div class="all-total">
          ...
          <div v-if="!isEdit" class="goPay" :class="{ disabled: selCount === 0 }" @click="clearCart">结算({{ selCount }})</div>
          <div v-else class="delete" @click="delGoods">删除{{ selCount }}</div>
        </div>
      </div>
    </div>
...
export default {
  ...
  computed: {
    ...
    ...mapGetters('cart', ['cartTotal', 'selCount', 'selPrice', 'isCheckedAll']),
    login () {
